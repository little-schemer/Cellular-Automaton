...
opt = Option
  ...
  <*> strOption   (short 'n' <> value "n"    <> metavar "m or n")
  <*> option auto (short 's' <> value state  <> metavar "Int")
...
drawModel fd (cells, _, clrs) = Pictures $ V.toList $ V.imap drawCell $ V.convert cells
